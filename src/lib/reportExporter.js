  ...
  buildCsv(state) {
    ...
    state.rows.forEach((row, i) => {
      const cells = [
        ...
        ReportExporter.numCell(row.nSubjects),
        ReportExporter.numCell(row.meanGrade),
        ReportExporter.numCell(row.nFailedGrades),
        row.program,
        row.enrollHist,
        row.prevStanding,
      ]
    ...
    return JSON.stringify(
      {
        ...
        bands: meta.bands,
        contract: {
          inputs: '11 in order: 8 float32 [N,1] + 3 string [N,1]',
        ...
        rows: state.rows.map((row, i) => {
          ...
          return {
            ...
            features: {
              ...
              n_subjects_t: isFinite(row.nSubjects) ? row.nSubjects : null,
              mean_grade_t: isFinite(row.meanGrade) ? row.meanGrade : null,
              n_failed_grades_t: isFinite(row.nFailedGrades) ? row.nFailedGrades : null,
